...
3. Because we did not provide any constraints for the Kubernetes scheduler, if you have a healthy cluster with additional nodes ready, a new pod should be created to replace our freshly deleted one. However, since only our original node hosts the data we just entered into our app, lets see what happens for our users...looks like everything is gone:
...
The `mon` block in the specification refers to Ceph monitors, which are described as the "brains" of the cluster. They are responsible for managing metadata necessary to store and retrieve your data. Ensuring monitors are healthy is vital to a healthy cluster. As with many other distributed computing platforms, use an odd number of monitors to ensure the highest availability. This is related to the concept of [quorum](https://en.wikipedia.org/wiki/Quorum_(distributed_computing)), which is very important to understand if use any distributed computing platforms. In most cases, this should be set to one per node.
