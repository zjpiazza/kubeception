...
Lets do a quick experiment to see the pitfalls that come with hostPath volumes. We are going to deploy a sample "todo" application, using the Helm chart published at the following address: [todo - Artifact Hub](https://artifacthub.io/packages/helm/nicholaswilde/todo). The application is pretty straightforward: a web interface to input a list of To-Dos.
...
4. Before we can install our todo application, we will provision the Persistent Volume and Persistent Volume Claim ahead of time. Use `kubectl apply` to create the required objects:
...
5. Now that we have created our namespace volumes, let us deploy the application using the following command:
...
3. Because we did not provide any constraints for the Kubernetes scheduler, if you have a healthy cluster with additional nodes ready, Kubernetes will immediately spring into action and  create a new  pod to replace our freshly deleted one. However, since only our original node hosts the data we just entered into our app, lets see what happens for our users...looks like everything is gone:
...
Okay, so what the heck just happened? Well if you're as confused as I was when I started writing this, you're not alone. So what I actually expected to happen here was that once the original pod has been killed, the Kubernetes scheduler would just fail to schedule a new pod because that pod was using a Persistent Volume set to hostPath storage. What actually happens is more insidious: Kubernetes happily schedules the new pod, without a care in the world for where the pod should reside. When we created our Persistent Volume, we specified the path on the host on which to mount. The data present in these directories is "out of band" as far as Kubernetes is concerned.
...
However, I want to quickly address a few points before we continue to the setup procedure. There are some hardware requirements associated with Rook and I want to ensure that you don't proceed any further if your cluster does not meet those requirements. Rook supports Kubernetes versions v1.25 through v1.30, so if you installed your cluster recently, you should be good here. The Rook operator also mandates the presence of specific Linux Kernel modules, however most modern Linux distributions should have these already.
...
Before we do anything, we must first install the Kubernetes operator for Rook. As with most operator installations, you have two options here:
- Install via standard YAML manifests
- Install via Helm

I always prefer installation from Helm because invariably you are going to want to upgrade the operator down the line, so using Helm charts just makes this process so much simpler.
...
As with our previous Helm installation, you should see a short message displayed in your console once the installation is successfully completed. Now lets use the operator to actually provision storage.

## Create Ceph cluster

The first step here is to provision the Ceph cluster. Below is the manifest I am currently using in my installation:
    ...
    spec:
      ...
      storage:
        useAllNodes: true
        useAllDevices: true
        # Only create OSDs on devices that match the regular expression filter
...
The `mon` block in the specification refers to Ceph Object Storage Daemons (OSDs). Ceph OSDs are the fundamental components of a Ceph storage cluster, responsible for storing data, handling data replication, recovery, backfilling, and rebalancing within the cluster. Each OSD runs on a storage device and communicates with other OSDs to ensure data redundancy and availability. By distributing data across multiple OSDs using the CRUSH algorithm, Ceph achieves fault tolerance and scalability, enabling the storage cluster to grow and manage large volumes of data efficiently. There are many reasons why you might want multiple OSDs per node in a cluster, but in our cases, that would be unnecessary. So we use a simple topology with three nodes and one OSD per node.

The `storage` block is also very important here. Rook provides different ways to specify which nodes and block devices should be registered to the cluster. The simplest method is to set `storage.useAllNodes` and `storage.useAllDevices` to `true` and then use the `storage.deviceFilter` parameter to target specific block devices. Since I am using all NVME drives for my Ceph cluster, I use the device name which corresponds to the first NVME drive installed in the node. If you were using SATA drives for your cluster, you would use `storage.deviceFilter=sda`.

Ensure that you have adjusted these parameters to match your hardware configuration and create the cluster: `kubectl apply -f ceph-cluster.yaml`. There are a number of things that can go wrong here, so you need to monitor the installation carefully. Firstly, check the status of the `CephCluster` resource by issuing the command `kubectl get cephcluster`. Once the `phase` is set to `Ready`, that means that the cluster was successfully provisioned. The installation can take quite a while to complete, so I would recommend monitoring the pods created in the namespace. If there are any provisioning issues, you will likely see that by viewing pod logs.

## Create Ceph Block Pool

A Ceph block pool is a logical grouping of storage within a Ceph cluster, used to manage block storage efficiently. It allows for customized data replication, erasure coding, and CRUSH rules for optimized data distribution and fault tolerance. Block pools can be configured with performance settings and quotas to control resource usage. They are essential for providing block storage to applications like virtual machines and databases.
...
As in previous steps, once this object has been created, check the status to ensure that the provisioning is successfully using `kubectl get cephblockpool` and examining the `phase` attribute.


## Create Storage Class

We are now ready to create our Storage Class and actually start using the platform. Although not required, I would recommend setting the `is-default-class` annotation to `true` here, so that you don't need to explicitly provide a Storage Class name in your applications.
...
Give yourself a round of applause: you have successfully configured a Ceph cluster using Rook in your cluster.

## Todo App v2

We are now going to use our shiny new Ceph cluster for provisioning Persistent Volumes for the todo application we tested with earlier. If you set the newly created Storage Class as default, the only change needed to the chart values is to remove the `persistence.Data.existingClaim` parameter. We are simplfy going to re-run the install command and go through the node failure simulation we completed earlier. Let's see what happens when the workload is shifted onto new nodes:

![Todo Ceph](./todo-ceph.png)

Zero data loss! Woo. Now that Ceph is handling our data replication needs, we no longer need to worry about nodes going offline and losing access to our data.
...
In the enterprise world, the integrity and availability of data is arguably the most important factor when it comes to your I.T. infrastructure. Many large companies and government instituions are using Rook to manage Ceph clusters on a massive scale. In fact, [Ceph is currently being used by CERN for a 65 petabye storage cluster](https://ceph.com/community/new-luminous-scalability/). If one of the largest research instituions in the world trusts this storage solution, I'm pretty sure it can candle my puny 3 terrabyte cluster with ease.
