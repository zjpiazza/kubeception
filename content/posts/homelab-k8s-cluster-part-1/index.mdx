...
Although Kubernetes has been around since all the way back in 2014, it took a few years of development before it really exploded. I became aware of it many years later, and accepted my first job opportunity using Kubernetes in 2020. Like most people, my first expose to it was as a managed service via a cloud provider.
...
But nowadays, organizations don't typically issue PCs to employees and laptops rein supreme. This is great for homelab enthusiasts because it means there is a huge supply of these computers sitting around collecting dust. And some amazing deals to be had, considering their capabilities.
...
For storage, my recommendation would be to get nodes with at least two SSDs installed in each node. Ideally, these drive configurations should be identical between all the nodes you are using for the cluster. This will enable us to configure a clustered storage like Rook/Ceph, which will help improve disaster recovery and developer experience. We will be covering this topic in a later post.
...
At this stage, you should have a working Kubernetes cluster on which you can deploy your applications. I mentioned earlier in this article that one way Kubernetes distributions often differ is when it comes to networking. If you are used to working with Kubernetes on the public cloud, you might notice if you try to create a service of type `LoadBalancer`, that you will not have an IP address allocated for your service and the command will error out. This is because the LoadBalancer service type, by design, links the service definition with an external load balancing service. When you try to create the service, a request is forwarded to the load balancing service for a new instance. Once the external load balancer is up, then the link is established and traffic can flow in.
